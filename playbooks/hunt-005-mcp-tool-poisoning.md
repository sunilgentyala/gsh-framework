...
**Framework:** Governed Security Hunting (GSH) v1.6.0
...
**Implementation Identity Gate (v1.7.0):** the schema hash above only covers the *interface* a server advertises - name, description, parameter schema. A server can keep that exact interface while its executable, script, or dependency lock changes underneath the same `--server-cmd`, i.e. "same exposed description, different real configuration." `capture` additionally resolves every local file the launch command references (interpreter + entrypoint script) and one adjacent dependency-lock file (`requirements.txt`, `poetry.lock`, `package-lock.json`, etc.), hashes each one, and stores that alongside the tool schema in the same `approval`-gated baseline. Before launching the wrapped server in *any* mode, `gsh-mcp-proxy.py` recomputes that identity and refuses to start if it does not match the approved baseline - the swapped implementation is never executed at all, not just quarantined after connecting. This closes a gap flagged in independent third-party review of this proxy (falsification test: approve a benign server, replace its script under the same command reference while preserving every tool definition, confirm the digest mismatch blocks the launch). It does not cover container/image digests or remote endpoint/TLS identity - see `adapters/mcp_proxy.py`'s module docstring for the full known-limitations list.
